...
DEBIANPATH="/data/local/debian/chroot"
...
download_and_execute_script() {
	...
	if [ -e "$DEBIANPATH/../start_debian.sh" ]; then
		echo -e "\e[1;33m[!] Script already exists: /$DEBIANPATH/../start_debian.sh\e[0m"
		echo -e "\e[1;33m[!] Skipping download...\e[0m"
	else
		wget -O "$DEBIANPATH/../start_debian.sh" "https://raw.githubusercontent.com/LinuxDroidMaster/Termux-Desktops/main/scripts/chroot/debian/start_debian.sh"
		if [ $? -eq 0 ]; then
			success "Script downloaded successfully: $DEBIANPATH/../start_debian.sh"
			progress "Setting script permissions..."
			chmod +x "$DEBIANPATH/../start_debian.sh"
